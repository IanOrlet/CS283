...
//printf("i just returned the wrong value");
    return rc;
}
...
void trim_whitespace2(char *str) {
    if (!str || *str == '\0') return;
    char *old = str, *new = str;
    int space = 0;
    while (isspace((unsigned char)*old)) old++;
    while (*old) {
        if (isspace((unsigned char)*old)) {
            if (!space) {
                *new++ = ' ';  
                space = 1;
            }
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if (io_size < 0) {  
		   ...
		   if (io_size >= 0 && io_size < RDSH_COMM_BUFF_SZ) {
        io_buff[io_size] = '\0';
		 
    } 
        io_buff[RDSH_COMM_BUFF_SZ - 1] = '\0'; 
			trim_whitespace2(io_buff);
        ...
        if (strcmp(io_buff, "stop-server") == 0) {
    	  ...
    	  char *dir = io_buff + 3;
        trim_whitespace2(dir); 

        if (chdir(dir) != 0) {
        send_message_string(cli_socket, "error: failed to change directory\n");
        } 
        	send_message_eof(cli_socket);
        continue;
        }	
...
int send_message_string(int cli_socket, char *buff){
    ...
    sent= send(cli_socket, buff, msg, 0);
    if (sent != msg) {
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    memset(pids_st, 0, sizeof(pids_st));    
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (WIFEXITED(pids_st[i]) && WEXITSTATUS(pids_st[i]) != 0) {
